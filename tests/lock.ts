...
describe("lock", () => {
  ...
  it("create voting escrow locker, lock time 1 year, min poll threshold 51% , min. tokens to start poll 100", async () => {
    if (rpc == "https://api.devnet.solana.com") {
      console.log("running on devnet");
      await program.methods.lockNew(0, day, new BN(0), 51, 25, min, "Monolith", "MONO")
        .accountsStrict({
          signer: wallet.publicKey,
          auth,
          lock,
      ...
      // await program.methods.lockNew(0, day, new BN(0), 51, 25, min, "SOON")
      await program.methods.lockNew(0, new BN(5), new BN(0), 51, 25, min, "Monolith", "MONO")
        .accountsStrict({
          signer: user1.publicKey,
          auth,
          //user: user1Pda,
  ...
  //       const debug = await program.account.lock.fetch(lock);
  //       console.log(debug);
  //       const dbg = await program.account.proposal.fetch(proposal);
  //       console.log(dbg);
  //     });
  // });
  ...
  //       const debug = await program.account.lock.fetch(lock);
  //       console.log(debug);
  //       const db = await program.account.proposal.fetch(proposal);
  //       console.log(db);
